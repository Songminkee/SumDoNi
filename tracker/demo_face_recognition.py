...
def simple_demo(config,args,arc_model,detector,Faces):
    img_raw = cv2.imread(args.src_path)
    if args.is_resize:
        img_raw = cv2.resize(img_raw, (args.resize, args.resize),interpolation=cv2.INTER_LINEAR)
    names, boxes, det_scores, sim_scores = face_recognition(img_raw,arc_model,detector,Faces,
                                                                                            detect_threshold=config.detect_threshold,
                                                                                            sim_threshold=config.sim_threshold,
                                                                                            draw_img=False,
                                                                                            indivisual_threshold=False)

    for i in range(len(names)):
        print(f"name = {names[i]}, boxes = {boxes[i]}, detection_scroe = {det_scores[i]}, sim_scores = {sim_scores[i]}")
...
if __name__ == '__main__':
    ...
    # input
    parser.add_argument('--simple_demo', action ='store_true', help='Default is false, If you want to inference for img using simple demo use --simple_mode ')
    ...
    if args.simple_demo:
        simple_demo(opt,args,arc_model,detector,Faces)
    else:
        if args.img_mode:
